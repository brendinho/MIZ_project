...
SK_true_data <- fread("CaseDataTables/SK_cases.csv") |>
    group_by(date) |>
    tally(cases) |>
...
SK_vacc_data <- fread("CaseDataTables/UofT_vaccines.csv") |>
...
# ggplot(SK_true_data, aes(date,cases)) + geom_line() + scale_x_date(date_breaks = "1 month", date_labels =  "%b %Y")
VHA_total_pop_break_indices <- c(
    SK_true_data[date %in% seq(lubridate::ymd('2020-11-15'), lubridate::ymd('2021-01-15'), '1 day')][which(log_cases==max(log_cases, na.rm=T)), index],
    SK_true_data[date %in% seq(lubridate::ymd('2021-03-10'), lubridate::ymd('2021-03-20'), '1 day')][which(log_cases==min(log_cases, na.rm=T)), index],
    SK_true_data[date %in% seq(lubridate::ymd('2021-04-01'), lubridate::ymd('2021-04-15'), '1 day')][which(log_cases==max(log_cases, na.rm=T)), index]
)
...
gathering_interventions <- aggregate(
    ...
    dplyr::filter(
        Weekend.Effective.Date >= SK_true_data[, min(date)] &
            Weekend.Effective.Date <= SK_true_data[, max(date)]
    ) |>
...
total_cases_seg_regression <- segmented(
    lm(log_cases ~ index, data=SK_true_data),
    seg.Z = ~ index,
...
cases_fitted_data <- data.table(
        date = SK_true_data$date,
        cases_line = unname(fitted(total_cases_seg_regression))
    ) |>
    filter(cases_line > 0)

total_vacc_seg_regression <- lm(log_vaccines ~ index, data=SK_vacc_data)

vacc_fitted_data <- data.table(
        date = SK_vacc_data$date,
...
results_I <- ggplot(SK_true_data, aes(x=date)) +
    geom_rect(
        data = vacc_and_cases,
        aes(
            xmin=start_date_both_cases_vacc_increase, 
    ...
    geom_line(data=SK_vacc_data, aes(date, log_vaccines), colour="red") +
    ...
    labs(
        ...
        # title="Semi-log plot: Saskatchewan
    ...
    ggsave(sprintf("%s/SK_I_plot.png", PROJECT_FOLDER), height=5, width=10)
