...
Case_File <- fread("http://www.bccdc.ca/Health-Info-Site/Documents/BCCDC_COVID19_Regional_Summary_Data.csv") |> 
    group_by(Date) |> 
    summarise(n=sum(Cases_Reported)) |>
    mutate(Total = n)  # |>
    # select(-n)

New_Cases <- Case_File |> pull(n)
First_Record_Date <- Case_File |> pull(Date) |> min() |> as.Date()
num_observations <- length(New_Cases)

raw_reporting_delays <- rbind(delay_data, delay_data_2020_09_22, delay_data_2021_01_05) |> 
    mutate(reporting_delay = as.numeric(time_to_report)) |>
...
ggplot(raw_reporting_delays, aes(x=reporting_delay)) +
    ...
    ggsave(sprintf("~/Documents/GitHub/MIZ_project/Graphs/Delay_Distros/BC_reporting_distribution.png"), width=13)
...
mcmc_samples_file_name <- paste0("~/Documents/GitHub/MIZ_project/BC_mcmc_samples_", gsub(" ", "_", as.character(Sys.time())), ".rds")
...
results_D <- ggplot(Summ[var=="D"], aes(x=date)) +
    ...
    ggsave(paste0("~/Documents/GitHub/MIZ_project/BC_D_plot_", gsub(" ", "_", as.character(Sys.time())), ".png" ), height=5, width=10)
...
results_I <- ggplot(Summ[var=="I"], aes(x=date)) +
    ...
    theme_bw() +
    labs(x="Date", y="New Cases") +
    ggsave(paste0("~/Documents/GitHub/MIZ_project/BC_I_plot_", gsub(" ", "_", as.character(Sys.time())), ".png" ), height=5, width=10)
