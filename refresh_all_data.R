...
interventions <- read.xlsx(
    ...
    dplyr::filter(Entry.ID %in% c(
        ...
        sprintf("NT%03i", c(016, 116, 170, 175)), # 4
        sprintf("NS%03i", c(009, 082, 047, 132, 257, 303)), # 6
        sprintf("NU%03i", c(012, 064, 114, 168, 249, 004, 050, 111, 167, 189, 196, 211, 223, 262, 347)), # 15
        sprintf("ON%03i", c(100, 009, 249, 343, 474, 660, 756)), # 7
        sprintf("PE%03i", c(020, 012, 119, 022, 146, 204, 213)), # 7
        sprintf("QC%03i", c(046, 043, 138, 033, 351, 579, 641, 735, 775)), # 9
        sprintf("SK%03i", c(026, 024, 096, 021, 157)), # 5
        sprintf("YK%03i", c(004, 051, 081, 010, 120))  # 5
    ))
...
if(! file.exists(file.path(PROJECT_FOLDER, "Classifications/interventions.csv")))
{
    interventions <- read.xlsx(
        ...
        dplyr::filter(Entry.ID %in% c(
            ...
            sprintf("NS%03i", c(009, 082, 047, 132, 257, 303)), # 6
            sprintf("NU%03i", c(012, 064, 114, 168, 249, 004, 050, 111, 167, 189, 196, 211, 223, 262, 347)), # 15
            sprintf("ON%03i", c(100, 009, 249, 434, 474, 660, 756)), # 7
            sprintf("PE%03i", c(020, 012, 119, 022, 146, 204, 213)), # 7
            sprintf("QC%03i", c(046, 043, 138, 033, 351, 579, 641, 735, 775)), # 9
            sprintf("SK%03i", c(026, 024, 096, 021, 157)), # 5
            sprintf("YK%03i", c(004, 051, 081, 010, 120))  # 5
...
Daycares <- interventions[grepl("daycares", tolower(type))]
Schools <- interventions[grepl("education", tolower(type))]
WFH <- interventions[grepl("home", tolower(type))]
...
{
    DATA %>% vdplyr::rename_with(tolower) %>%
        dplyr::filter(jurisdiction == JURIS) %>%
        split(1:nrow(.)) %>%
