...
if(! file.exists(file.path(PROJECT_FOLDER, "covid-19-intervention-scan-data-tables-en.xlsx") %>% file.exists()) )
{
    download.file(
...
fwrite(
    rbind(
        TEI %>% dplyr::mutate(type = "tightened"),
        EEI %>% dplyr::mutate(type = "eased")
    ), 
    file="~/Desktop/eased.csv"
)

TEI_plot <-TEI %>% # rbind(TEI, EEI) %>%
    gg_vistime(col.event="what", col.group="jurisdiction", col.start="date.implemented", col.end="effective.until", show_labels=FALSE) +
    scale_x_datetime(
        breaks = seq(
