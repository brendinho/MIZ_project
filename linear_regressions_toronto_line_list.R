...
if(! file.exists("CaseDataFiles/raw_toronto_data.csv"))
{
    ...
    fread("https://ckan0.cf.opendata.inter.prod-toronto.ca/download_resource/e5bf35bc-e681-43da-b2ce-0242d00922ad?format=csv") |>
        ...
        fwrite("raw_toronto_fsa_data.csv")
}

writeLines("\nfixing Toronto data")
raw_toronto_fsa_data <- fread("raw_toronto_fsa_data.csv") |>
...
###################################################### SURVEY VALUES
{
    average_values <- data.table(attribute=as.character(), value=as.character(), wave_1_report_delay=as.character(), wave_2_report_delay=as.character(), 
        total_report_delay=as.character())
    ...
    # scrapped Kullback-Leibler code before EOF
}
