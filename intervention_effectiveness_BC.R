...
BC_true_data <- fread("CaseDataTables/BC_cases.csv") |>
    group_by(date) |>
    tally(cases) |>
...
BC_vacc_data <- fread("CaseDataTables/UofT_vaccines.csv") |>
...
VHA_total_pop_break_indices <- c(
    BC_true_data[date %in% seq(lubridate::ymd('2020-03-15'), lubridate::ymd('2020-04-01'), '1 day')][which(log_cases==max(log_cases, na.rm=T)), index],
    BC_true_data[date %in% seq(lubridate::ymd('2020-05-15'), lubridate::ymd('2020-06-15'), '1 day')][which(log_cases==min(log_cases, na.rm=T)), index],
    BC_true_data[date %in% seq(lubridate::ymd('2020-11-01'), lubridate::ymd('2020-12-01'), '1 day')][which(log_cases==max(log_cases, na.rm=T)), index],
    BC_true_data[date %in% seq(lubridate::ymd('2021-01-15'), lubridate::ymd('2021-02-15'), '1 day')][which(log_cases==min(log_cases, na.rm=T)), index],
    BC_true_data[date %in% seq(lubridate::ymd('2021-04-01'), lubridate::ymd('2021-04-15'), '1 day')][which(log_cases==max(log_cases, na.rm=T)), index]
)
...
gathering_interventions <- aggregate(
    ...
    dplyr::filter(
        Weekend.Effective.Date >= BC_true_data[, min(date)] &
            Weekend.Effective.Date <= BC_true_data[, max(date)]
    ) |>
...
total_cases_seg_regression <- segmented(
    lm(log_cases ~ index, data=BC_true_data),
    seg.Z = ~ index,
    psi = VHA_total_pop_break_indices
)

cases_fitted_data <- data.table(
        date = BC_true_data$date,
        cases_line = unname(fitted(total_cases_seg_regression))
    ) |>
    filter(cases_line > 0)

total_vacc_seg_regression <- lm(log_vaccines ~ index, data=BC_vacc_data)

vacc_fitted_data <- data.table(
        date = BC_vacc_data$date,
...
results_I <- ggplot(BC_true_data, aes(x=date)) +
    geom_rect(
        data = vacc_and_cases,
        aes(
            xmin=start_date_both_cases_vacc_increase, 
    ...
    geom_line(data=BC_vacc_data, aes(date, log_vaccines), colour="red") +
    ...
    labs(
        ...
        # title="Semi-log plot: British Columbia
    ...
    ggsave(sprintf("%s/BC_I_plot.png", PROJECT_FOLDER), height=5, width=10)
