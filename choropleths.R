...
# REMEMBER TO MIMIC SEYED"S YELLOW COLOUR SCHEME
...
breaks_waves <- c(
    break_dates$date[c(5,7)], # the last two minima
    break_dates$date[2] + floor(as.numeric(break_dates$date[3] - as.numeric(break_dates$date[2])))/2 # the midpoint of the first trough
)
...
) %>% merge(canada %>% select(date, fit, type), by=c("date", "type"))
...
ggsave(canada_wave_break_point_analysis, file=file.path(PROJECT_FOLDER, "Graphs/canada_wave_break_points.png"), width=10, height=7)
